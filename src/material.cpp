...
void Material::calculate_xs_no_depletion(Particle& p) const
{
  if (p.type_ == Particle::Type::neutron) {
    this->calculate_neutron_xs_no_depletion(p);
  } else if (p.type_ == Particle::Type::photon) {
    this->calculate_photon_xs(p);
  }
}

void Material::calculate_neutron_xs_no_depletion(Particle& p) const
{
...
void Material::calculate_neutron_xs(Particle& p) const
{
  ...
  for (int i = 0; i < nuclide_.size(); ++i) {
    ...
    for (int r = 0; r < DEPLETION_RX_SIZE; r++) 
      reaction[r] += atom_density * nuclide_micro.reaction[r];
  }
  ...
  p.macro_xs_.nu_fission = macro.nu_fission;
  for (int r = 0; r < DEPLETION_RX_SIZE; r++) 
    p.macro_xs_.reaction[r] = reaction[r];
